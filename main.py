...
from youtube import get_vid_id, make_playlist, make_service, add_item_to_playlist
import os
import logging
...
if client_secret is None:
    ...
    raise ValueError(
        "SP_CLIENT_SECRET is not set in the environment variables.")
...
def main(pl_id: str):
    try:
        ...
        if not items_name:
            logging.error(
                f"Failed to fetch playlist data for playlist ID: {pl_id}")
            return None
